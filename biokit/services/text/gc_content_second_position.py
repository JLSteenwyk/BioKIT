...
class GCContentSecondPosition(Text):
    ...
    def run(self):
        ...
        if self.verbose:
            for record in records:
                ...
                second_position_char = self.get_second_position_char(record, second_position_char)

                _, matches = self.find_matches(''.join(second_position_char))

                print(f"{record.id}\t{round(len(matches)/len(second_position_char), 4)}")
