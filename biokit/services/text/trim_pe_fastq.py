...
class TrimPEFastQ(Text):
    ...
    def run(self):
        ...
        with open(self.fastq1) as in_handle1, open(self.fastq2) as in_handle2:
            ...
            zip(FastqGeneralIterator(in_handle1), FastqGeneralIterator(in_handle2)):
        ...
        if len(good_reads_paired1) != 0:
            output_file_paired_1 = re.sub(".fastq$|.fq$", "_paired_trimmed.fq", self.fastq1)
        ...
        if len(good_reads_paired2) != 0:
            output_file_paired_2 = re.sub(".fastq$|.fq$", "_paired_trimmed.fq", self.fastq2)
        ...
        if len(good_reads_unpaired1) != 0:
            output_file_unpaired_1 = re.sub(".fastq$|.fq$", "_unpaired_trimmed.fq", self.fastq1)
            with open(output_file_unpaired_1, 'w') as output_fastq_file_name_1:
                output_fastq_file_name_1.write('\n'.join(good_reads_unpaired1))
        if len(good_reads_unpaired2) != 0:
            output_file_unpaired_2 = re.sub(".fastq$|.fq$", "_unpaired_trimmed.fq", self.fastq2)
